export const RETRO_CLASSES = {
  ...
  buttonBase:
    "retro-button inline-flex items-center justify-center font-[Inconsolata] select-none leading-none transition-colors duration-100 active:translate-y-px disabled:opacity-60 disabled:cursor-not-allowed",
  chipButton:
    "rounded-sm px-2.5 py-1.5 text-xs sm:text-sm border border-[var(--retro-border-soft)] bg-[var(--retro-surface)] text-[var(--retro-text-chrome)] hover:bg-[var(--retro-inset)]",
  primaryButton:
    "rounded-sm px-3 py-2 text-sm border border-[#255A34] bg-[var(--retro-accent-green)] text-white hover:brightness-95",
  ghostButton:
    "rounded-sm px-3 py-2 text-sm border border-transparent bg-transparent text-[var(--retro-text-primary)] hover:bg-[var(--retro-surface)]",
  panelLink:
    "rounded-sm px-2.5 py-1 text-xs sm:text-sm border border-[var(--retro-border-soft)] bg-[var(--retro-inset-alt)] text-[var(--retro-accent-blue-text)] hover:bg-white",
  dockItemBase:
  ...
  stampFocusOffset:
    "focus-visible:ring-offset-[var(--retro-desktop)]",
  terminalShellShadow:
    "shadow-[0_16px_36px_rgba(28,33,50,0.14)]",
  scrollContentArea:
    "min-h-0 overflow-auto terminal-scroll",
} as const;
